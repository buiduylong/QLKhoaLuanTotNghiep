...
class Detai extends Migration
{
    ...
    public function up()
    {
        Schema::create('detai', function (Blueprint $table) {
            $table->increments('id_detai')->unsigned();
            $table->string('ten_detai');
            $table->integer('thanhvien_detai')->unsigned();
            $table->foreign('thanhvien_detai')->references('id')->on('thanhvien')->onDelete('cascade');
            $table->integer('nam_detai')->unsigned();
            $table->foreign('nam_detai')->references('id_nam')->on('nam')->onDelete('cascade');
            $table->integer('ky');
            $table->integer('nhom');
            $table->timestamps();
        });
